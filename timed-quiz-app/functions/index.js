...
// Removed version marker as requested
      ...
      // Attach participation certificate PDF (only if enabled in quiz_settings)
      try {
          ...
          msg.attachments = [
            {
              ...
              filename: `Participation_Certificate_${(name || "Participant").replace(/\s+/g, "_")}.pdf`,
              type: "application/pdf",
              disposition: "attachment",
            },
          ];
          console.log("Certificate PDF generated and attached");
        } else {
          console.log("Certificate disabled in quiz_settings — skipping attachment");
        }
      } catch (certError) {
        console.error("Failed to generate certificate PDF, sending email without it:", certError);
      }
